...
def send_email(to_addresses, cc_addresses, subject, body):
    """Invio email via SMTP (unico sistema).
    - Usa header From (visualizzato) = MAIL_FROM_HEADER
    - Usa envelope-from (SMTP MAIL FROM) = MAIL_ENVELOPE_FROM
    - Supporta TO e CC; come destinatari SMTP usa TO+CC.
    """
    # accetta stringhe "a@;b@" oppure liste
    def _to_list(x):
        if not x:
            return []
        if isinstance(x, str):
    ...
    to_list = _to_list(to_addresses)
    cc_list = _to_list(cc_addresses)
    ...
    to_list = _unique(to_list)
    cc_list = _unique(cc_list)
    ...
    # RCPT TO = TO + CC
    rcpt_list = _unique(to_list + cc_list)
    ...
    with smtplib.SMTP(SMTP_SERVER, int(SMTP_PORT), timeout=20) as server:
        server.ehlo()
        # opzionale: TLS + AUTH (se il relay lo richiede)
        if SMTP_USE_TLS:
            server.starttls()
            server.ehlo()
...
def render_email_box(*, context_key: str, nc_id: str, operation: str = "Aggiornamento NC", default_to: str = "", default_cc: str = ""):
    ...
    if st.button("📨 Invia email", key=f"{context_key}_send"):
        ...
        with st.spinner("Genero email con Gemini e invio via SMTP..."):
            subject, body = generate_email_subject_body(operation, nc or {}, ac_list or [])
            send_email(to_list, cc_list, subject, body)
...
def generate_email_subject_body(operation: str, nc: dict, ac_list: list[dict] | None = None) -> tuple[str, str]:
    """Oggetto deterministico + corpo generato con Gemini (con prompt fisso richiesto)."""
    ac_list = ac_list or []
    op = (operation or "").lower()

    # subject (semplice e stabile)
    ncn = str(_get_any(nc or {}, "nonconformance_number", "NONCONFORMANCE_NUMBER", "id", default="")).strip()
    if not ncn:
        ncn = "?"

    subject_prefix = "aggiornamento NC"
    if "nuova" in op and "nc" in op:
        subject_prefix = "nuova NC"
    subject = f"NC {ncn} – {subject_prefix}"

    body = generate_email_body_with_gemini(nc or {}, ac_list)
    return subject, body
...
def view_modifica_nc(df_nc: pd.DataFrame, df_ac: pd.DataFrame):
    ...
    render_email_box(
        ...
        default_to=str(_get_any(row.to_dict() if hasattr(row,'to_dict') else (row or {}), "email_address", "EMAIL_ADDRESS", "owner", "OWNER", default=""))
    )
